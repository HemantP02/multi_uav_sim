...
int main(int argc, char **argv)
{
    ros::init(argc, argv, "setpoint_node3");
    ros::NodeHandle nh("~");

    ros::Subscriber status_sub = nh.subscribe("/setpoint_node1/uav1/loop_complete", 10, statusCallback);
    ros::Publisher setpoint_pub = nh.advertise<geometry_msgs::PoseStamped>("/uav3/mavros/setpoint_position/local", 10);
    ros::Rate rate(2.0);

    // Define waypoint 1
    geometry_msgs::PoseStamped waypoint1;
    waypoint1.header.frame_id = "map";
    waypoint1.pose.position.x = 5;
    waypoint1.pose.position.y = 7;
    waypoint1.pose.position.z = 10;
